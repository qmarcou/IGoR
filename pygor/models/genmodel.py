...
class GenModel:
	"""Generative model class containing model topology and realization probabilities.

	Similar to IGoR's C++ GenModel class.
	"""
	...
	def read_model_parms(self,filename):
		"""Reads a model graph structure from a model parms file. 
...
class Rec_Event:
	""" Recombination event class containing event's name, type, realizations, etc...

	Similar to IGoR's C++ RecEvent class.
	"""
...
class Adjacency_List:
	...
	Similar to IGoR's C++ Adjacency_List class. Stores adjacent events (parents and children) to a given event.	
	"""
	def __init__(self):
		self.children = list()
		self.parents = list()
...
def compute_average_distribution(event_name,model,averaging_list = None):
	...
	Alternatively a list of ancestors can be provided, such that the distribution is only averaged over these ancestors.
	"""
	for event in model.events:
		if event.name == event_name:
		    ...
		    if event.event_type!="DinucMarkov":
		        ...
		        sum_axes.remove(find(np.asarray(dimension_names_list)==event.nickname))
		        if averaging_list!=None:
		            #for dependance_name in averaging_list:
		                #sum_axes.remove(find(np.asarray(dimension_names_list)==dependance_name))
		            for dimension_name in dimension_names_list:
		                if dimension_name!=event.nickname and all(np.asarray(averaging_list)!=dimension_name):
		                    sum_axes.remove(find(np.asarray(dimension_names_list)==dimension_name))
...
def read_marginals_txt( filename , dim_names=False):
	"""Reads a model marginals file.

	Returns a tuple containing a dict containing the individual events probabilities indexed by the events nicknames and a dict containing the list of dimension names/ordering for each event. 
	"""
	with open(filename,'r') as file:
