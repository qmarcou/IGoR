    ...
    for event in model1.events:
        if event.name == event_name:
            ...
            if event.event_type!="DinucMarkov":
                ...
                final_array*=log2(reshaped_event_marginals+pseudo_count)
                if debug_output:
                    return -final_array
                else:
...
def compute_markov_steady_entropy(dinuc_event,model1,model2,event_cluster,dimension_list,dimension_names_list,insertion_event):
    ...
    for i in range(0,tot_dimension):
        ...
        for [transition_matrix,mono_nt_vect,conditional_nt_mat] in [(transition_matrix_1,mono_nt_vect_1,conditional_nt_mat_1),(transition_matrix_2,mono_nt_vect_2,conditional_nt_mat_2)]:
            ...
            stat_dist = divide(stat_dist ,sum(stat_dist))#although should already be a unit vector
            
            #print("Stationnary distribution:" + str(stat_dist))

            mono_nt_vect[i] = stat_dist
            conditional_nt_mat[i] = diag(asarray(stat_dist).reshape(4))
            #print(asarray(stat_dist).reshape(4))
            #print(diag(asarray(stat_dist).reshape((4))))

        mono_nt_cross_entropy[i] = nansum(multiply(mono_nt_vect_1[i],log2(mono_nt_vect_2[i]))) #SHOULD PROBABLY ADD PSEUDOCOUNTS
        cross_entropy_production_rate[i] = nansum(nansum(multiply(conditional_nt_mat_1[i]*transition_matrix_1,log2(transition_matrix_2))))
...
def compute_type_model_DKL(type_list,model1,model2):
    total_DKL=0
    for event in model1.events:
        if(any(asarray(type_list)==event.event_type)):
            total_DKL+=compute_event_DKL(event.name,model1,model2)
    return total_DKL    
