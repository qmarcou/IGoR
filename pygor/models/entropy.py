...
def compute_cross_entropy_subparts( event_name , model1 , model2 , debug_output = False ):
	"""Returns the cross entropy -sum(p_model1*log(p_model2)) for the specified recombination event."""
		...
		if event.name == event_name:
		    ...
		    while not explored:
		        ...
		        for parent in parents_names:
		            if not model1.edges.has_key(parent):
		                continue
		    ...
		    for ev in event_cluster:
		        dimension_list.append(model1.marginals[0][ev.nickname].shape[-1])
		        dimension_names_list.append(ev.nickname)

		    if event.event_type!="DinucMarkov":

		        final_array = np.ones(tuple(dimension_list))
		        ...
		        for ev in event_cluster:
		            if not model1.edges.has_key(ev.name):
		                event_parents = []
		            else:
		            ...
		            for e in event_cluster:
		                if( (event_parents.count(e.name)>0) or (ev.name == e.name) ):
		                    dimension_list.append(model1.marginals[0][e.nickname].shape[-1])# same as getting the number of realizations
		                else:
		            ...
		            swapped_dimension_names =  copy.deepcopy(model1.marginals[1][ev.nickname])
		            ...
		            while(not is_sorted):
		                for i in range(0,len(swapped_marginal.shape)):
		                    if(i == len(swapped_marginal.shape)-1):
		                        is_sorted = True
		                        break
		                    elif (find(np.asarray(dimension_names_list) == swapped_dimension_names[i]) > find(np.asarray(dimension_names_list) == swapped_dimension_names[i+1])):
		        ...
		        if not model1.edges.has_key(event_name):
		            event_parents = []
		        else:
		            event_parents = model1.edges[event_name].parents
		        dimension_list = []
		        for e in event_cluster:
		            if( (event_parents.count(e.name)>0) or (event_name == e.name) ):
		                dimension_list.append(model1.marginals[0][e.nickname].shape[-1])# same as getting the number of realizations
		            else:
		                dimension_list.append(1)

		            swapped_marginal = copy.deepcopy(model2.marginals[0][event.nickname])
		            swapped_dimension_names =  copy.deepcopy(model2.marginals[1][event.nickname])
		        ...
		        reshaped_event_marginals = swapped_marginal.reshape(tuple(dimension_list))
		        ...
		        pseudo_count = reshaped_event_marginals[reshaped_event_marginals!=0].min()*10**(-5)
		        final_array*=np.log2(reshaped_event_marginals+pseudo_count)
		        if debug_output:
		            return -final_array
		        else:
		    ...
		    if event.event_type=="DinucMarkov":
		        ...
		        for junction in junction_list:
		            ...
		            for ev in model1.events:
		                if ev.event_type=="Insertion" and ev.seq_type==junction:
		                    insertion_event = ev
		                    junction_found = True
		                    break
		            if not junction_found:
		                print("INSERTION JUNCTION NOT FOUND: " + junction)
		            dinuc_cross_entropy.append((junction,compute_markov_steady_entropy(event,model1,model2,event_cluster,dimension_list,dimension_names_list,insertion_event)))
...
def compute_markov_steady_entropy(dinuc_event,model1,model2,event_cluster,dimension_list,dimension_names_list,insertion_event):
	"""Compute the Dinucleotide markov model cross entropy.

	The entropy is approximated assuming the markov chain is at steady state.	
	"""
	final_array = np.ones(tuple(dimension_list))
	#print(final_array.shape)
	...
	swapped_dimension_names =  copy.deepcopy(model2.marginals[1][dinuc_event.nickname])
	...
	flattened_marginals_1 = flattened_marginals_1.reshape((tot_dimension,4,4))

	mono_nt_vect_1 = np.ndarray((tot_dimension),dtype=np.matrix)
	conditional_nt_mat_1 = np.ndarray((tot_dimension),dtype=np.matrix)

	flattened_marginals_2 = swapped_marginal
	flattened_marginals_2 = flattened_marginals_2.reshape((tot_dimension,4,4))

	mono_nt_vect_2 = np.ndarray((tot_dimension),dtype=np.matrix) 
	conditional_nt_mat_2 = np.ndarray((tot_dimension),dtype=np.matrix) 

	mono_nt_cross_entropy = np.ndarray((tot_dimension),dtype=float)# entropy of steady state distribution
	cross_entropy_production_rate = np.ndarray((tot_dimension),dtype=float)# entropy of steady state distribution


	for i in range(0,tot_dimension):
		transition_matrix_1 = np.mat(flattened_marginals_1[i,:,:])
		transition_matrix_2 = np.mat(flattened_marginals_2[i,:,:])
		for [transition_matrix,mono_nt_vect,conditional_nt_mat] in [(transition_matrix_1,mono_nt_vect_1,conditional_nt_mat_1),(transition_matrix_2,mono_nt_vect_2,conditional_nt_mat_2)]:
		    #print(transition_matrix)
		    [eigenValue,eigenVect] = np.linalg.eig(transition_matrix.transpose())
		    ...
		    stat_dist = np.divide(stat_dist ,sum(stat_dist))#although should already be a unit vector
		    
		    #print("Stationnary distribution:" + str(stat_dist))

		    mono_nt_vect[i] = stat_dist
		    conditional_nt_mat[i] = np.diag(np.asarray(stat_dist).reshape(4))
		    #print(np.asarray(stat_dist).reshape(4))
		    #print(np.diag(np.asarray(stat_dist).reshape((4))))

		mono_nt_cross_entropy[i] = np.nansum(np.multiply(mono_nt_vect_1[i],np.log2(mono_nt_vect_2[i]))) #SHOULD PROBABLY ADD PSEUDOCOUNTS
		cross_entropy_production_rate[i] = np.nansum(np.nansum(np.multiply(conditional_nt_mat_1[i]*transition_matrix_1,np.log2(transition_matrix_2))))
	...
	if model1.edges.has_key(dinuc_event.name) and model1.edges.parents.count(insertion_event.name):
		insertion_dependent = True

		
	cross_entropy = np.ndarray((tot_dimension),dtype=float)
		...
		for i in range(0,tot_dimension):
		    ...
		    for ins_real in insertion_event.realizations:
		        ...
		        if(ins_real.value > 0):
		            ...
		            L_ins_entropy+=cross_entropy_production_rate*(ins_real.value -1) #add cross entropy production for every markov step
...
def compute_event_entropy(event_name , model,debug_output = False):
	...
	return compute_cross_entropy_subparts(event_name , model , model , debug_output)

def compute_event_DKL(event_name , model1 , model2 , debug_output = False):
	"""Compute the Kullback Leibler divergence for the specified event and provided models.

	Returns  D_KL(model1||model2) for the considered event.
	"""
...
def compute_type_model_DKL(type_list,model1,model2):
	"""Compute Kullback Leibler divergence associated with events of supplied types."""
	total_DKL=0
	for event in model1.events:
		if((np.asarray(type_list)==event.event_type).any()):
		    total_DKL+=compute_event_DKL(event.name,model1,model2)
	return total_DKL    
