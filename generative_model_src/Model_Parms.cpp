...
Model_Parms::Model_Parms() {
	this->events = *(new list <Rec_Event*>());
	this->edges = *(new unordered_map <Rec_Event_name , Adjacency_list >());

}

/*
 * Construct a Model_Parms from a list of event and initialize connectivities
 */
Model_Parms::Model_Parms(list <Rec_Event*> event_list){
	this->events = event_list;
	this->edges = *(new unordered_map<Rec_Event_name,Adjacency_list>());
	size_t event_identifier = 0;
	for(list<Rec_Event*>::const_iterator iter = this->events.begin() ; iter != this->events.end() ; ++iter){
...
Model_Parms::Model_Parms(const Model_Parms& other){
	...
	for(unordered_map <Rec_Event_name , Adjacency_list >::const_iterator iter = other.edges.begin() ; iter != other.edges.end() ; ++iter){
		...
		for(list<Rec_Event*>::const_iterator jiter = (*iter).second.children.begin() ; jiter != (*iter).second.children.end() ; ++jiter){
			for(list<Rec_Event*>::const_iterator kiter = this->events.begin() ; kiter != this->events.end() ; kiter++){
				if((**kiter)==(**jiter)){
		...
		for(list<Rec_Event*>::const_iterator jiter = (*iter).second.parents.begin() ; jiter != (*iter).second.parents.end() ; ++jiter){
			for(list<Rec_Event*>::const_iterator kiter = this->events.begin() ; kiter != this->events.end() ; ++kiter){
				if((**kiter)==(**jiter)){
...
bool Model_Parms::add_event(Rec_Event* event_point){
	...
	this->edges.emplace(event_point->get_name(),*(new Adjacency_list()));
	return 1;
}

list<Rec_Event*> Model_Parms::get_parents(Rec_Event* event) const{
	return edges.at(event->get_name()).parents;
}

list<Rec_Event*> Model_Parms::get_children(Rec_Event* event) const{
 ...
 */
list<Rec_Event*> Model_Parms::get_roots() const{
	list<Rec_Event*> root_list = *(new list<Rec_Event*>);
	for ( list <Rec_Event*>::const_iterator iter = this->events.begin(); iter != this->events.end(); ++iter){
 ...
 */
queue<Rec_Event*> Model_Parms::get_model_queue() const{

	list<Rec_Event*> model_roots = this->get_roots();
	list<Rec_Event*> events_copy_list = this->events;


	queue<Rec_Event*> model_queue = *(new queue<Rec_Event*>);
	...
	if(events_copy_list.size() == model_roots.size()){
		...
		for(list<Rec_Event*>::const_iterator iter=model_roots.begin(); iter!=model_roots.end(); ++iter){
	...
	unordered_map<Rec_Event_name,Rec_Event*>* processed_events_point = new unordered_map<Rec_Event_name,Rec_Event*>;
	...
	list<Rec_Event*>::iterator iter = events_copy_list.begin();
	...
	while(!events_copy_list.empty()){
		...
		list<Rec_Event*> event_parents = this->edges.at( (*iter)->get_name() ).parents;
		bool parents_processed = 1;
		for(list<Rec_Event*>::const_iterator jiter = event_parents.begin() ; jiter != event_parents.end() ; ++jiter){
...
Rec_Event* Model_Parms::get_event_pointer(const Rec_Event_name& event_name) const{
...
unordered_map<tuple<Event_type,Gene_class,Seq_side>, Rec_Event*> Model_Parms::get_events_map(){
	unordered_map<tuple<Event_type,Gene_class,Seq_side>, Rec_Event*> events_map;
	for(list<Rec_Event*>::const_iterator iter = this->events.begin() ; iter != this->events.end() ; ++iter ){
...
void Model_Parms::write_model_parms(string filename){
	...
	outfile<<"@Edges"<<endl;
	for(list<Rec_Event*>::const_iterator iter=events.begin() ; iter != events.end() ; ++iter){
		list<Rec_Event*> children = edges[(*iter)->get_name()].children;
		for(list<Rec_Event*>::const_iterator jiter=children.begin() ; jiter!=children.end() ; ++jiter){
...
void Model_Parms::read_model_parms(string filename){
	...
	if(line_str == string("@Event_list")){
		...
		while(line_str[0] == '#'){
			...
			if(event == string("Insertion")){
				unordered_map<string,Event_realization> event_realizations = *(new unordered_map<string,Event_realization>());
				getline(infile,line_str);
				while(line_str[0]=='%'){
				...
				Insertion* new_event_p =  new Insertion(event_class  , event_realizations);
				...
				Deletion* new_event_p = new Deletion(event_class , event_side , event_realizations);
				new_event_p->set_priority(priority);
				new_event_p->set_nickname(nickname);
				this->add_event(new_event_p);
			}
				...
				Gene_choice* new_event_p = new Gene_choice(event_class  , event_realizations);//TODO construct event before and use add realization instead?
				new_event_p->set_priority(priority);
				new_event_p->set_nickname(nickname);
				this->add_event(new_event_p);
			}
			else if(event== string("DinucMarkov")){
				Dinucl_markov* new_event_p = new Dinucl_markov(event_class);
				new_event_p->set_priority(priority);
				new_event_p->set_nickname(nickname);
				this->add_event(new_event_p);
				getline(infile,line_str);
	...
	if(line_str == string("@ErrorRate")){
		...
		if(line_str == string("#SingleErrorRate")){
			getline(infile,line_str);
			Single_error_rate* err_rate_p =  new Single_error_rate(stod(line_str));
